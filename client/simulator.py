...
class Simulator:
    def __init__(self, cln: client.Client, ctx: context.Context):
        self.client = cln
        self.context = ctx
        self.signal = False
        self.auction = auction.Auction(self.context.min_price, self.context.max_price)
    ...
    def run(self):
        while not self.signal:
            ...
            if bid_response.status == "error":
                print(">>>> send_bid_request(): status == error")
                continue
            ...
            if impression:
                ...
                if not res:
                    print(">>>> send_impression(): ack == false")
